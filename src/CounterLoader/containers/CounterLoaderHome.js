...
function getSectionHeight(element) {
  const { height } = element.getBoundingClientRect(); // the height pf each ul tag //1790
  const { childElementCount } = element; // how many li tags  //10
...
export default function CounterLoaderHome() {
  ...
  useEffect(() => {
    if (countRef.current && countRef2.current && loaderRef.current) {
      const transformAmount = getSectionHeight(countRef.current);
      let tl = gsap.timeline({ defaults: { ease: "power2", duration: 2, delay: .7 } }).timeScale(2)

      const sequence1 = [...Array(6)].fill("").flatMap((_, index) => {
        // tl.to(countRef.current, { y: `-${transformAmount * (index + 1)}px` })
        //   .to(countRef2.current, { y: `-${transformAmount * (index + 1)}px` }, "-=.8")
        tl.to(countRef2.current, { y: `-${transformAmount * (index + 1)}px` })
          .to(countRef.current, { y: `-${transformAmount * (index + 1)}px` }, "-=.8")

      })

      gsap.fromTo(countRef3.current, { y: "100%" }, { delay: 14, y: 0 })
  ...
  useEffect(() => {
    ...
    tl.to(countRef.current, { opacity: 0, delay: 15 })
      ...
      .to(countRef3.current, { opacity: 0 }, "<+=.2") //2 sec after the first animation
      .to(imageRef.current, { scale: 1.2 }, "<")
      ...
      <CursorManager>
        ...
        <div className="loader-container" ref={loaderRef}>
          <div className="counter-container">
            <ul className="counter-list" ref={countRef3}>
              <li><h3>1</h3></li>
            </ul>
          </div>
