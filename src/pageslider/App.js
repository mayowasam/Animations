...
function App() {
  ...
  useEffect(() => {
    ...
    console.log('quotesList', quotesList.children);
  ...
  const slideRight = (index, duration, multiple = 1) => {
    ...
    console.log('imageList.children[index]', imageList.children[index]);
  ...
  const nextSlide = () => {
    ...
    console.log('activeSlide after', activeSlide);

    // if (activeSlide > 1) {
      // testimonials.map((_, i) => slideLeft(i, 1, activeSlide));
      testimonials.map((_, i) => {
        console.log("i", i);
        return slideLeft(i, 1, activeSlide)
      })
